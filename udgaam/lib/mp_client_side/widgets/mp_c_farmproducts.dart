...
class FarmWiseProduct extends StatefulWidget {
  final String farmerId;

  const FarmWiseProduct({super.key, required this.farmerId});

  @override
  State<FarmWiseProduct> createState() => _FarmWiseProductState();
}

class _FarmWiseProductState extends State<FarmWiseProduct> {
  List<Map<String, dynamic>> products = [];
  bool isLoading = true;
  String? sortOption = 'Name';
  String farmerName = "";

  // Define the getS3Url function for image URLs
  ...
  Future<void> fetchProducts() async {
    ...
    try {
      // Fetch farmer's name once using widget.farmerId
      ...
      for (var product in productList) {
        product['farmer_name'] = fetchedFarmerName; // Assign the fetched name
      }

      setState(() {
        products = productList;
        farmerName = fetchedFarmerName;
        isLoading = false;
      });
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(farmerName.isEmpty ? 'Loading...' : farmerName),
